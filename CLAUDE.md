...
| `DESIGN-SYSTEM.md`           | **Superseded — carries a banner saying so (9 Sep 2026).** Kept only because ~80 references still resolve into it; never extend it. Its _structure_ rules — page patterns, density intent, copy law — are restated in `DESIGN.md` §10, which is where to read them. Deleting the file is a pending bulk-reference cleanup |
...
| Lists                           | **FlashList v2** (2.0.2, what `npx expo install` resolves for SDK 57)                                                                                                                          | Long workout histories, food diaries, and §8.2's 100-client coach dashboard. Pinned here since the stack was written but only **installed by `coach-dashboard/01`**, the first task with a list long enough to need it — every earlier scan list is a `ScrollView` and says so in its own file. ⚠️ **v2 deleted `estimatedItemSize`.** §25.8 ("FlashList needs `estimatedItemSize` or scroll performance collapses") is a **v1** rule: the prop is absent from 2.0.2 entirely, because v2's recycler measures rows itself. What §25.8 protects is honoured one layer down — a row pins its own `minHeight` to a measured constant, so it reports a stable size on first layout. `overrideItemLayout` is not a substitute: in v2 it hands back only `{ span }`, never a size. Most FlashList examples online are still v1; check the version before trusting one. ⚠️ **Not a native module, unlike every other `@shopify/*` row here** — 2.0.2 ships `dist`/`src` only, no `android`/`ios`, no podspec, no `codegenConfig`, no config plugin. So it is **OTA-able** and needs no dev-client rebuild; v1's `FlashList` was native and this is the second v1 assumption that does not survive v2. |
...
| Audio (rest timer, voice notes) | **expo-audio**                                                                                                                                                                                 | `expo-av` is deprecated; do not use it. Ships inside `expo`, MIT, no bill at any volume (§3.4.1 step 2). Its first real consumer is the rest-timer completion tone (`rest-timer/04`), not voice notes — imported in exactly one file, `apps/mobile/src/features/workouts/lib/rest-timer-audio.ts`, which owns the audio session and the one player the app has. That file, never a caller, is where `setAudioModeAsync` sets `playsInSilentMode` and `shouldPlayInBackground`; a second opinion about the session category is how the tone goes silent on a locked phone, which is §25.7's pitfall and the only moment the tone matters. The `app.config.ts` plugin entry is a tuple, not the bare string `expo install` prints: the plugin's defaults request the microphone and Android `RECORD_AUDIO`, which a rest timer must not. **Native: dev-client rebuild, never an OTA (§25.11).**                                                                                                                                                                                                                                                                                                  |
| Camera                          | **expo-camera**                                                                                                                                                                                | Form-check capture + barcode scanning                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| Media picker                    | **expo-image-picker**                                                                                                                                                                          | Gallery uploads                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| Video trim & compression        | **react-native-video-trim** (8.1.0)                                                                                                                                                            | `upload-client/01`'s trim to ≤90s and `/02`'s compress to ≤6MB per 30s, in one dependency. Free, OSS (MIT), New Architecture + RN 0.86, hardware-accelerated (`h264_videotoolbox` on iOS, `h264_mediacodec` on Android). **Use only the headless `trim({ startTime, endTime })` and `compress({ quality \| bitrate, width, height })`** — the package also exports `showEditor()`, a full-screen native trimmer, and calling it would put an undesigned screen in front of a client (rule 7a). CoachOS draws its own trim UI. ⚠️ **It bundles its own FFmpeg build, ~9MB of APK.** That is the honest price of client-side video work and it is app size, not §19's JS-bundle budget or §22's server cost. ⚠️ **`ffmpeg-kit-react-native` is retired (Jan 2025, binaries pulled) — do not adopt it, and distrust any tutorial that does.** `react-native-compressor` was the runner-up: +50KB on native APIs instead of FFmpeg, but it does not trim, and the capture path does not need it (see the note below). **Native: dev-client rebuild, never an OTA (§25.11).**                                                                                                                       |
...
| Screen keep-awake               | **expo-keep-awake**                                                                                                                                                                            | Held only while a session is actually `in_progress`, never for the whole route — a completed session left open would pin the screen against §19's battery budget. Used through the imperative API behind a serialized chain in `apps/mobile/src/features/workouts/hooks/useSessionKeepAwake.ts`, not the package's own `useKeepAwake`, whose cleanup releases without awaiting the in-flight activate and can leak the lock. Ships inside `expo` already, so no native change and **OTA-able**, unlike the rows above it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| Local DB                        | **expo-sqlite** + **Drizzle ORM**                                                                                                                                                              | Offline workout logging (`offline-sync` skill). Installed ahead of P08 by `providers-and-gates/02`, which opens its own `coachos-query-cache.db` for the TanStack Query persister. **The two connections are permanently separate, not pending consolidation** — this row previously said `phase-08-offline-core/local-database` would merge them, and that task looked at it and decided against: the mirror (`apps/mobile/src/db/`, `coachos.db`) and the read cache have independent invalidation, so `local-database/04`'s schema-mismatch drop discards the mirror without touching the cache, and `local-database/03`'s logout wipe clears the cache without waiting on the mirror's outbox-empty gate. Merging would couple `meta.schema_version` to `QUERY_CACHE_BUSTER` for no gain. Drizzle arrives with P08 — the persister uses one hand-written key/value table and needs no ORM. **Native: dev-client rebuild, never an OTA (§25.11).**                                                                                                                                                                                                                                          |
| Connectivity                    | **expo-network**                                                                                                                                                                               | The one network-state source for the app (`phase-08-offline-core/connectivity/01`) — outbox flush-on-regain, prefetch gating, offline banners. Imported in exactly one file, `apps/mobile/src/lib/connectivity/store.ts`, which registers a single global listener behind `useConnectivity()`; a second listener anywhere reports the same change at a different moment and the app disagrees with itself. **Native: dev-client rebuild, never an OTA (§25.11).**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| Notifications                   | **expo-notifications**                                                                                                                                                                         | Push via EAS + APNs/FCM. Also the rest timer's backgrounded countdown (`rest-timer/03`) — a local, ongoing, silent notification on its own `rest-timer` channel. Imported in exactly two files: `features/onboarding/notification-permission.ts` _requests_ the permission, `features/workouts/lib/rest-timer-live-activity.ts` _presents_ with it and never requests. **No `expo-widgets`**: a real iOS Live Activity is reachable in SDK 57 through it (stable since SDK 56), but it needs a second native target and `@expo/ui/swift-ui` — the package §3.1.1 deliberately stripped. Revisit as a §3.4.1 decision, not inside a task.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| Analytics                       | **PostHog** (`posthog-react-native`, pinned 4.67.0)                                                                                                                                            | Product analytics + feature flags in one. Imported in exactly one file — `apps/mobile/src/lib/analytics/posthog.ts` — which exports `trackEvent()` and never the client (§20, `ANALYTICS.md` AN§1). **`posthog-react-native-session-replay` is deliberately NOT installed**: RN session replay needs that separate native module, so §20's "no session recording, ever" cannot be undone by a config flag or an OTA (`providers-and-gates/04`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| OS tracking flag                | **expo-tracking-transparency**                                                                                                                                                                 | Reads the iOS ATT status so the analytics gate can honour §20's "limit ad tracking" requirement (`ANALYTICS.md` AN§2.3). **Read only** — we never call `requestTrackingPermissionsAsync()` (no prompt, so no `NSUserTrackingUsageDescription` and no config-plugin entry) and never call `getAdvertisingId()`, because CoachOS collects no advertising identifier on either platform. Requesting either is a §27 decision.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| Crash/errors                    | **Sentry** (`@sentry/react-native`, pinned 8.25.0)                                                                                                                                             | Source maps uploaded in EAS build hook. Imported in exactly one file — `apps/mobile/src/lib/sentry.ts` — which exposes `initSentry()` and its `beforeSend` scrubber and never the client, the same rule the Analytics row states. **Pinned above Expo SDK 57's version map, which resolves `~7.11.0`**: that row is stale rather than curated — the same map still lists `sentry-expo ~7.0.0`, deprecated since SDK 50, and 7.11.0 predates both the 8.0.0 major and RN 0.86. 8.25.0 carries `@sentry/core` 10.73.0, one major line with apps/api's `@sentry/node` 10.70.0, so both scrubbers rebuild the same `ErrorEvent` shape from one allowlist. ⚠️ `expo-doctor` / `expo install --fix` reports this as a mismatch and offers the downgrade — refuse it. `@sentry/cli`'s build script is allowed in `pnpm-workspace.yaml`; it fetches the binary both source-map upload paths shell out to.                                                                                                                                                                                                                                                                                              |
...
| **react-native-video-trim**         | Free, OSS (MIT)                          | forever         | — trim and compress run on-device; no service, no per-call cost. This is why client-side, not managed. |
...
8. **FlashList v2 has no `estimatedItemSize`.** The prop existed in v1, and every tutorial still tells you to set it; 2.0.2 deleted it — passing it is a type error, not a tuning mistake. v2's recycler measures rows itself. What the old rule protected is now the row's job: pin a uniform `minHeight` on the row component so the first layout pass measures a stable height instead of settling over several frames. `overrideItemLayout` is not a substitute — in v2 it returns only `{ span }`, never a size (§3.1, `coach-dashboard/01`).
...
13. **Do not cap a gallery-picked video with `expo-image-picker`'s `allowsEditing`.** It crops and rotates; it is not a timeline trim, and on iOS it silently caps at 10 minutes rather than your number. The capture path _is_ safely capped natively — `expo-camera`'s `recordAsync({ maxDuration, maxFileSize })` stops the recording itself — so the trim library is needed for the gallery path and nothing else.
14. **iOS suspends JS the instant the screen locks**, so anything already on a lock screen freezes at the value it was posted with. A notification body reading "1:30 left" is wrong by however long the phone stayed in a pocket — and wrong in the direction that costs the client something. Put an absolute instant on the surface ("until 10:42"), never a relative one only a live runtime could keep true. Android, which can re-post, is the exception rather than the rule.
...
- [ ] **Free-form client tags.** §8.2 asks the coach dashboard to "filter by tag/status", and `DATABASE.md` DB§5.1 has no `tags` column. `coach-dashboard/02` resolved "tag" to the two facets a client row already carries — `client_profiles.status` and `client_profiles.goal` — which covers the v1 filtering need without a schema change. Genuine free-form tagging ("comp prep", "post-natal", "Tuesday 6am group") is a different feature: a `client_tags` table, a per-coach tag vocabulary, authoring UI on the client-detail screen, and a place in the export and purge orders. **Trigger:** a coach asking for it by name in the §23 ship-gate-1 pilot. Scoped through `phase-01-data-layer` when it happens, never invented inside a screen task.
- [x] ~~**Program assignment: live-reference over snapshot.**~~ → **Live-reference confirmed by Ammar, 8 September 2026** (pre-phase-09 audit, step 2). A template edit reaches every assigned client immediately; `programs.version` stays a change counter; `workout_sessions.program_snapshot` is written only when a client _starts_ a session (DB§14.6), which is P09's work. Proven both directions by `apps/api/src/routers/__tests__/assignments.bulk-edit.test.ts`. Original entry, kept for the reasoning: `assignment/00` resolved that a client's assignment points at the coach's program template directly — a template edit is visible to every assigned client immediately, and `programs.version` is a change counter, not a per-session pin. Neither `DATABASE.md` nor this file stated the model outright; the resolution was inferred at planning time from §8.4's bulk-edit acceptance criterion, which is incoherent under the snapshot alternative. Defensible, but it is a product decision made without Ammar's direct sign-off — worth a look. Full rationale: `apps/api/src/features/programs/versioning.md`.
- [ ] **`workout_sessions.status` last-write-wins is order-dependent for two lagging offline devices.** DB§14.3 resolves this one field by comparing `updated_at`, but migration 0021's `touch_updated_at` trigger sets `updated_at = now()` on every update — so once any write lands, the row's comparison basis is the server clock, not the winning device's change time. Resolution stays order-independent for the case DB§14.3 is written for (one lagging device against an online coach, whose timestamp is truthful) and becomes order-dependent only when two lagging offline devices race the same session. `sync-engine/04` treats DB§14.5's session claim (mechanism 3: `active_device_id`/`claimed_at`, which makes a second device refetch server truth before it can write) as already preventing that scenario, and ships no schema change. The alternative is a `status_updated_at` column the trigger does not own. Pinned by a test in `apps/api/src/lib/workout-session-upsert.test.ts` so it cannot regress silently — revisit if a real two-device status conflict is ever observed.
