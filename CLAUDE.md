...
| Canvas drawing          | **@shopify/react-native-skia** (2.6.2, what `npx expo install` resolves for SDK 57)                                                                                                            | Arcs and paths: `ProgressRing` and the charts §7.4's **victory-native** draws, which renders through Skia — so the choice was "one renderer or two", never "none". §3.4.1: free, MIT, no bill at any volume. **Native: dev-client rebuild, never an OTA (§25.11).** ⚠️ `react-native-svg` stays for four icon sites; `MacroBar` uses neither.          |
